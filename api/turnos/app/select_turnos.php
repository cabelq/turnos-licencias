...
if ( validaSelect( $selectDestino ) && validaOpcion ( $opcionSeleccionada ) ) {
	...
	//$select = "SELECT * FROM horarios h WHERE h.hora_puesto NOT IN (SELECT t.hora_puesto FROM turnos t where fecha_turno = ? AND fecha_anulado IS NULL)"; //Consulto todos los turnos asignados para ese dia.
	  $select = "SELECT * FROM horarios h WHERE h.hora_puesto NOT IN (SELECT t.hora_puesto FROM turnos t where fecha_turno = ? AND fecha_anulado IS NULL) AND publico = 1"; //Consulto todos los turnos asignados para ese dia.
